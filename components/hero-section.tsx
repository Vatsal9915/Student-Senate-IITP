...
export function HeroSection() {
  ...
  return (
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden ">
      ...
      <button
      onClick={prevSlide}
      className="absolute left-6 top-1/2 transform -translate-y-1/2 z-20 p-3 rounded-3xl bg-[#453CD5]/20 backdrop-blur-sm hover:bg-[#453CD5]/30 transition-all duration-200 group"
      >
      <ChevronLeft className="w-6 h-6 text-[#453CD5] group-hover:text-blue-700" />
      </button>
      <button
      onClick={nextSlide}
      className="absolute right-6 top-1/2 transform -translate-y-1/2 z-20 p-3 rounded-3xl bg-[#453CD5]/20 backdrop-blur-sm hover:bg-[#453CD5]/30 transition-all duration-200 group"
      >
      <ChevronRight className="w-6 h-6 text-[#453CD5] group-hover:text-blue-700" />
      </button>
      ...
      <div className="mb-8 flex justify-center">
        ...
        <div className="w-20 h-20 bg-gradient-to-br from-blue-400 to-[#453CD5] rounded-3xl flex items-center justify-center shadow-2xl animate-pulse">
      ...
      <h1 className="text-4xl md:text-6xl lg:text-7xl font-bold mb-4 bg-gradient-to-r from-gray-800 via-[#453CD5] to-gray-800 bg-clip-text text-transparent">
        Student  Technical Council 
      </h1>
      <h2 className="text-xl md:text-2xl lg:text-3xl font-medium mb-4 text-[#453CD5]">IIT Patna</h2>
      <p className="text-2xl md:text-3xl lg:text-4xl mb-8 font-semibold bg-gradient-to-r from-black via-[#453CD5] to-black bg-clip-text text-transparent">
      ...
      <p className="text-lg md:text-xl mb-12 max-w-3xl mx-auto text-gray-600 leading-relaxed">
        A reimagined platform with three specialized wings -  <span className="  bg-gradient-to-r from-[#453CD5] via-blue-700 to-[#453CD5] bg-clip-text text-transparent font-semibold text-2xl">DISHA, ARTHNITI, and TATVA </span> -  fostering career
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-6 max-w-4xl mx-auto">
        ...
        >
          ...
          style={{
          background: "radial-gradient(circle at 50% 50%, rgba(69,60,213,0.25) 0%, rgba(69,60,213,0.10) 60%, transparent 100%)",
          filter: "blur(30px)",
          }}
          />
          <div className="relative z-10 bg-white/10 rounded-3xl backdrop-blur-sm">
          <div className=" z-9 text-2xl mb-2">{stat.icon}</div>
          <div className="z-9 text-2xl md:text-3xl font-bold text-[#453CD5] mb-1">{stat.value}</div>
      ...
      <div className="w-6 h-10 border-2 border-[#453CD5] rounded-full flex justify-center">
        <div className="w-1 h-3 bg-[#453CD5] rounded-full mt-2 animate-pulse"></div>
      </div>
      </div>
    </section>
  )
